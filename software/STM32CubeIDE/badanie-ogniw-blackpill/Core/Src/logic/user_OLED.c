...
//	ILI9341_DrawText(5,  110, buffer, FONT4, WHITE, BLACK);
	...
	ILI9341_DrawRectangle(0, 180, ILI9341_SCREEN_WIDTH, FONT4h, BLACK);
	ILI9341_DrawText(5,  180, buffer, FONT4, WHITE, BLACK);

	// INA333_voltage
	snprintf(buffer, sizeof(buffer), "Napiecie ogniwa: %.2f V ", s.voltage);
	ILI9341_DrawRectangle(0, 210, ILI9341_SCREEN_WIDTH, FONT4h, BLACK);
	ILI9341_DrawText(5,  210, buffer, FONT4, WHITE, BLACK);

	// PWM "ADC: %.2f%%\r\n",
	snprintf(buffer, sizeof(buffer), "Prad ladowania: %.1fA", s.set_current_charge);
	ILI9341_DrawRectangle(0, 240, ILI9341_SCREEN_WIDTH, FONT4h, BLACK);
	ILI9341_DrawText(5, 240, buffer, FONT4, WHITE, BLACK);

	// PWM "ADC: %.2f%%\r\n",
	snprintf(buffer, sizeof(buffer), "Prad rozladowania: %.1fA", s.set_current_discharge);
	ILI9341_DrawRectangle(0, 270, ILI9341_SCREEN_WIDTH, FONT4h, BLACK);
...
void display_menu_main(void) {
    ...
    st.menu_current_ptr = (enum MenuScreen)(MENU_START + (st.enc_count % SCREENS_MENU_NUM));
...
void display_menu_current_charge(void) {
	...
	snprintf(buffer, sizeof(buffer), "%.1fA", st.set_current_charge_prev);
	x_pos = (ILI9341_SCREEN_WIDTH - ILI9341_GetTextWidth(buffer, FONT4)) / 2;
	y_pos = (ILI9341_SCREEN_HEIGHT - (2 * FONT4h))/2 + (2*FONT4h);
	ILI9341_DrawText(x_pos, y_pos, buffer, FONT4, YELLOW, BLACK);
}
...
void action_menu_current_discharge(void) {
	s.set_current_discharge = st.set_current_discharge_prev;
...
void action_menu_current_charge(void) {
	s.set_current_charge = st.set_current_charge_prev;
...
void display_bottom_bar(void) {
	...
	snprintf(buffer, sizeof(buffer), "%.1fA", s.set_current_charge);
	ILI9341_DrawText(x_pos, y_pos, buffer, FONT4, WHITE, BLACK);
	...
	snprintf(buffer, sizeof(buffer), "%.1fA", s.set_current_discharge);
